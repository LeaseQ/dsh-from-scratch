...
export const steps = [
  ...
  /* ========== 第二模块：可追溯事件流 ========== */
  {
    ...
    prose:
      ...
      "<p>为了先有个整体印象，下面这张图把这一章要讲的东西摆到一起：中间是那条 append-only 的日志，它是本章围绕的真相源；四周几个方框，则是从同一条日志读出来的不同能力，模型历史由 <code>deriveMessages</code> 派生，回放、分叉、续跑、可观测各取所需。带着这张图往下读，后面每一节其实都是在把其中一条箭头补全。</p>" +
      LOG_READS_SVG +
